...
The goal is to maximize the expected reward $$ \mathop{argmax}\limits_{a}\quad q_\ast(a) $$. Since we do not know the $$ q_\ast(a) $$, we need to estimate it. The simplest method is via the *sample-average* method. That is, we sample each arm equally and calculate the estimated action values. Therefore, we have the action value at time $$ t $$:

\begin{equation}
\label{eq:valu-action}
Q_t(a) =  \frac{\sum_{i=1}^{t-1}R_i \cdot \mathbbm{1}_{A_{i=a}}}{\sum_{i=1}^{t-1}\mathbbm{1}_{A_{i=a}} }
\end{equation}
