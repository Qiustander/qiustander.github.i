...
The goal is to maximize the expected reward $ argmax_a q_\ast(a) $. Since we do not know the $ q_\ast(a) $, we need to estimate it. The simplest method is via the *sample-average* method. That is, we sample each arm equally and calculate the estimated action values. Therefore, we have the action value at time $ t $:
\begin{equation}
